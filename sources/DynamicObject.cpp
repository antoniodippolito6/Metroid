...
void DynamicObject::jump(bool spaceBar)
{
	if(!midair())
...
void DynamicObject::resolveCollisions()
{
	...
	std::vector<std::pair<Object*, Direction>> hits;
	for (auto obj : sortedByContactTime)
		if (DynamicRectVsRect(sceneCollider(), vel(), obj.first->sceneCollider(), cp, cn, ct))
		{
			...
			hits.push_back({ obj.first, normal2dir(cn) });
		}

	// perform hits
	for (auto h : hits)
	{
		h.first->hit(this, h.second);
		hit(h.first, inverse(h.second));
	}
}
